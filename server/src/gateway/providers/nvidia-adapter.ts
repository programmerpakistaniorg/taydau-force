...
export class NvidiaAdapter implements ProviderAdapter {
  ...
  readonly defaultBilling = 'FREE_CREDITS' as const;

  isConfigured(): boolean {
    return Boolean(config.nvidia.apiKey && config.nvidia.apiKey.trim().length > 0);
  }

  async listModels(): Promise<string[]> {
    if (!this.isConfigured()) return [];
    try {
      const url = `${config.nvidia.baseUrl}/models`;
  ...
  async validateConnection(): Promise<{ ok: boolean; error?: string }> {
    if (!this.isConfigured()) {
      return { ok: false, error: 'NVIDIA_API_KEY is not configured in environment.' };
    }
    try {
      const models = await this.listModels();
      if (models.length > 0) return { ok: true };
      return { ok: false, error: 'Could not fetch models from NVIDIA NIM API.' };
    } catch (err: any) {
  ...
  async execute(
    ...
    const baseUrl = config.nvidia.baseUrl || 'https://integrate.api.nvidia.com/v1';
    ...
    if (!res.ok) {
      ...
      const error: any = new Error(`NVIDIA NIM HTTP ${res.status}: ${errText}`);
  ...
  parseError(err: any, headers?: Headers | Record<string, string>): ParsedProviderError {
    ...
    const message = err.message || 'Unknown NVIDIA NIM error';
    ...
    if (statusCode === 429) {
      return {
        ...
        isTransient: true,
        message: `NVIDIA NIM rate limit exceeded (HTTP 429). Retry after ${retryAfterMs || 30000}ms.`,
      };
    }
    ...
    if (statusCode === 401 || statusCode === 403) {
      return {
        ...
        isModelNotFound: false,
        isTransient: false,
        message: 'NVIDIA NIM authentication failed (HTTP 401/403). Invalid API key.',
      };
    }

    if (statusCode === 402 || message.toLowerCase().includes('credit') || message.toLowerCase().includes('billing')) {
      return {
        ...
        isModelNotFound: false,
        isTransient: false,
        message: 'NVIDIA NIM free credits exhausted (HTTP 402).',
      };
    }
    ...
    if (statusCode === 404) {
      return {
        ...
        isTransient: false,
        message: 'Requested NVIDIA NIM model not found or deprecated.',
      };
    }
    ...
    if (statusCode >= 500) {
      return {
        ...
        isModelNotFound: false,
        isTransient: true,
        message: `NVIDIA NIM service outage (HTTP ${statusCode}).`,
      };
    }
