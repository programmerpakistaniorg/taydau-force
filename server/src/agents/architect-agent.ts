...
- Do NOT alter or replace this core tech stack.
- Architecture must support all validated requirements and link to the implementation tasks.
- Define a modular file structure (e.g., app/main.py, app/models.py, app/schemas.py, app/api/endpoints.py, tests/test_api.py).
- implementationSpec (string) must clearly define:
  ...
  5. Security & Concurrency Considerations (e.g., atomic inventory updates, input validation)
  6. Automated Test Strategy for Pytest
...
Return the result strictly as a valid JSON object with EXACTLY this top-level structure:
{
  ...
  "fileStructure": ["app/main.py", "app/models.py", ...],
